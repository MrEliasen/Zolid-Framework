...
class users_install extends AppController
{
	...
	protected function action_createadmin()
	{
		...
		if( $stmt->rowCount() > 0 )
		{
			...
			if( $this->forceLogin( $this->model->connection->lastInsertId() ) )
			{
				...
				@unlink(ROOTPATH . DS . 'app' . DS . 'views' . DS . 'users' . DS . 'install.pdt');
				@unlink(ROOTPATH . DS . 'app' . DS . 'controllers' . DS . 'users_install.php');
			}
	...
	private function makeConfig( $filename, array $find, array $replace )
	{
		$filepath = ROOTPATH . DS . 'config' . DS . $filename . '.php';
